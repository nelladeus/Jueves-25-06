...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dashboard) {
    ...
    function tickReadouts() {
      const brakeVal = Math.floor(Math.random() * 100);
      const throttleVal = Math.floor(Math.random() * 100);
      const steeringVal = Math.floor(Math.random() * 180) - 90; // -90° a 90°
  ...
  window.switchHardware = function (key) {
    ...
    if (mediaElement) {
      ...
      setTimeout(() => {
        ...
        document.getElementById('hw-vs').innerText = hardwareData[key].vs;
